(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))e(t);new MutationObserver(t=>{for(const a of t)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&e(l)}).observe(document,{childList:!0,subtree:!0});function s(t){const a={};return t.integrity&&(a.integrity=t.integrity),t.referrerPolicy&&(a.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?a.credentials="include":t.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function e(t){if(t.ep)return;t.ep=!0;const a=s(t);fetch(t.href,a)}})();document.querySelector("#app").innerHTML=`
...
`;document.addEventListener("DOMContentLoaded",()=>{const r=new IntersectionObserver(s=>{s.forEach(e=>{e.isIntersecting&&e.target.classList.add("animate")})},{threshold:.1});document.querySelectorAll(".slide-in-left, .slide-in-right, .fade-up, .feature-card, .stat-card, .form-field").forEach(s=>{r.observe(s)});const i=document.querySelector(".partners-track");i&&(i.addEventListener("mouseover",s=>{const e=s.target.closest(".partner-logo");e&&e.classList.add("partner-logo-hover")}),i.addEventListener("mouseout",s=>{const e=s.target.closest(".partner-logo");e&&e.classList.remove("partner-logo-hover")}))});document.addEventListener("DOMContentLoaded",()=>{const r=new IntersectionObserver(s=>{s.forEach(e=>{if(e.isIntersecting&&(e.target.classList.add("animate"),e.target.classList.contains("stat-number"))){const t=parseInt(e.target.getAttribute("data-target")),a=2e3,l=0,o=t/(a/16);let c=l;const n=setInterval(()=>{c+=o,c>=t?(e.target.textContent=t+(e.target.getAttribute("data-suffix")||""),clearInterval(n)):e.target.textContent=Math.floor(c)+(e.target.getAttribute("data-suffix")||"")},16)}})},{threshold:.1});document.querySelectorAll(".slide-in-left, .slide-in-right, .fade-up, .feature-card, .stat-card, .form-field, .stat-number, .section-title").forEach(s=>{r.observe(s)});const i=document.querySelector(".partners-track");i&&(i.addEventListener("mouseover",s=>{const e=s.target.closest(".partner-logo");e&&e.classList.add("partner-logo-hover")}),i.addEventListener("mouseout",s=>{const e=s.target.closest(".partner-logo");e&&e.classList.remove("partner-logo-hover")}))});
